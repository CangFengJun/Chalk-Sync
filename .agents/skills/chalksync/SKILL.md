...
Use the repository's `scripts/chalksync-profile` command to build a local study pack.
...
- Let the user assign either local `ds` or `gpt` Model Profile to either stage. Never silently switch profiles after an error.
- Read API keys only from the Git-ignored `.local-secrets/profiles/*.env` files. Never place keys in tracked files, commands, logs, provenance, or generated notes.
...
1. Run `scripts/chalksync-profile doctor` and report missing prerequisites. Use `doctor --probe` only when the user asks to test endpoint authentication.
...
4. Run the worker stage with the user's selected profile. It must detect layout first and extract board and slide regions before chunk analysis. DeepSeek supports only `sync`; use `--mode batch-submit` only with a profile whose endpoint implements Files, Batch, and Responses.
5. For supported Batch jobs, stop after submission and record the course path. On a later request, run `collect` until it completes.
6. Run `finalize` with the user's selected final profile and the course's public video URL.
